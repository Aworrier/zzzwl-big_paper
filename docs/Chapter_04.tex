\chapter{面向 CSMA 网络的连续传输机制设计与分析优化}
\label{chapter4:CSMAST}

在前一章验证了连续传输机制在时隙 Aloha 网络中的性能增益后，本章进一步将其扩展至具备载波侦听能力的 CSMA 网络，提出 CSMAST 接入机制。与 Aloha 协议中基于等长离散时隙的建模假设不同，CSMA 协议中空闲、碰撞与成功传输的状态持续时长并不完全等同为单位时隙，从而给系统的理论建模带来了新的挑战。
...
\subsection{网络吞吐量 $\lambda^{net}_{out}$ 的仿真结果与分析}
...
\begin{figure}[t]
	...
	\begin{subfigure}{0.49\textwidth}
		...
		\caption{不同 $\tau_S$ 下 $\lambda^{net}_{out}$ 随接入概率 $q$ 的变化}
	...
	\begin{subfigure}{0.49\textwidth}
		...
		\caption{网络吞吐量 $\lambda^{net}_{out}$ 随节点数量 $n$ 的变化}
