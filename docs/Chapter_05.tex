...
离散事件仿真器能够在给定协议配置和网络条件下精确评估系统性能，但其计算开销较高。以典型仿真配置（如 $n=100$ 个节点、$T=3\times10^5$ 个时隙）为例，单次样本点的仿真通常需要数十秒甚至数分钟。当需要在多协议、多参数组合及不同流量场景下进行大规模性能评估时，总体仿真时间将迅速累积。本节基于第\ref{subsec:dataset_construction}节构建的仿真数据集，设计并训练一种高效的多任务性能预测模型（Multi-Task Deep Neural Network, MT-DNN）。后文统一简称为性能预测模型。该模型通过共享特征表示并联合学习多个性能指标，实现对系统吞吐量、时延与公平性等关键指标的快速预测。相比传统仿真方法，该模型能够将性能评估时间显著降低，从而为后续的协议选择与参数优化提供高效支撑。
...
在均匀流量且强度递变的场景一中，本文对比了智能化策略与两类基线策略的综合效用表现，以评估各策略在时序变化的负载下的动态自适应能力。实验结果表明，在归一化平均到达率为$0.10$的极低负载阶段即窗口1和窗口10，网络碰撞概率极小，代表单一协议静态配置的基线1采用恒定传输概率$0.01$即可满足传输需求，此时三种策略的综合效用基本持平并均维持在$0.45$左右。然而随着负载强度的逐步上升，策略机制差异导致的性能分化尤为明显。特别是在归一化平均到达率分别激增至$0.70$的窗口4和$0.80$的窗口8的高负载冲击下，基线1由于采用恒定参数而完全无法适应剧烈增加的并发请求，导致其综合效用发生显著下跌并降至$0.35$至$0.38$。相比之下，代表单一协议内理论最优动态调参的基线2凭借对CBA协议参数的细粒度穷举寻优机制，有效缓解了高并发碰撞，将高负载期间的综合效用维持在$0.58$左右的中等水平。更为重要的是，本文提出的代理策略在整个中高负载波动期间展现出了最优且最稳定的性能，其综合效用始终保持在$0.6$以上的最高水平。这不仅彻底克服了静态配置策略在动态环境下的脆弱性，更突破了单一协议内最优参数搜索的性能天花板，充分证明了代理策略能够智能且精准地适配流量强度的动态变化，从而在复杂网络环境下实现系统综合效用的最大化。

在流量类型与强度联合变化的场景二中，本文进一步评估了各策略在复杂动态网络环境下的综合效用表现。观察实验结果可知，在处于极低负载区间的窗口1与窗口10，三种策略的综合效用表现基本一致并均维持在0.45左右。随着网络负载的时序演进，采用单一协议静态配置的固定SA基线策略再次暴露出抗并发冲击能力不足的缺陷，在面临窗口4与窗口8的高强度负载时，其综合效用发生显著跌落。仅在单一协议内进行动态寻优的CBA参数搜索基线策略虽在一定程度上缓解了高负载带来的性能衰退，但面对频繁切换的流量形态，其综合效用曲线呈现出明显波动，暴露出单一协议机制难以全面适配异构流量的局限性。相比之下，本文提出的代理策略展现出了卓越的跨场景泛化能力与高度的动态自适应性。无论底层流量类型如何在不同形态间交替更迭，亦或负载强度如何剧烈起伏，代理策略均能智能决策出最匹配当前网络状态的协议配置，在绝大部分窗口将其综合效用稳定维持在0.6以上的较高水平。特别是在窗口7至窗口10经历连续的流量类型与到达率双重剧变阶段，代理策略的效用表现依然平稳且显著优于两类基线策略，充分证明了其在异构多变网络环境下的全面性能优势。
