...
定义候选接入协议集合为 $\mathcal{P} = \{a_1, a_2,  a_3\}$，该集合具体包含 SA、SAST、CBA三种典型接入协议。每一种协议 $a \in \mathcal{P}$ 对应一组独立课配置的参数集合 $\mathcal{X}_p$。
...
\end{equation}
其中 $\lambda_{\mathrm{out}}$ 表示系统吞吐量，$D$ 表示平均数据包时延，$J_T$ 表示公平性（采用 Jain 指数刻画），$S$ 表示信令开销。所有指标均在本文第\ref{sec:performance_metrics}节上给出标准定义。上述指标集合兼顾业务性能与代价，便于在多协议、多参数配置下进行统一评估与对比。
上述各个性能指标的量纲存在显著差异，吞吐量单位为数据包/时隙，时延单位为时隙，公平性指标无量纲。此外，数值尺度差异也较大，例如，时延数值通常为数百上千个时隙，而吞吐量范围仅在0到1之间。若直接用于性能对比与决策分析，数值尺度较大的指标会掩盖数值尺度较小指标的影响，严重降低评估结果的客观性与准确性。为解决该问题，本文在同一对比组内对各项性能指标采用时延对数化+最小最大归一化处理，并将所有指标统一转换为效用型指标，即数值越大，对应系统性能越优，从而消除量纲与数值尺度差异带来的负面影响。具体而言，先对时延指标$D$采用$log(1+D)$进行对数化预处理，压缩其数值尺度，再将对数化后的时延与其他指标（$\lambda_{\mathrm{out}}, J_T, S$）一同实施最小最大归一化。
...
为验证所提综合评价方法的有效性与适用性，，后文实验部分定义三种典型业务场景，分别为吞吐量优先、时延优先、信令开销优先，各场景权重向量设置为$
	\mathbf{w}_{\mathrm{lambda}}=[0.6, 0.2, 0.1, 0.1]$,
	$\mathbf{w}_{\mathrm{D}}=[0.2, 0.6, 0.1, 0.1]$,
	$\mathbf{w}_{\mathrm{S}}=[0.2, 0.2, 0.1, 0.5]$。
...
从结果来看，吞吐量 $\lambda_{\mathrm{out}}$ 与信令开销 $S$ 等指标的决定系数均接近 $1$，表明模型能够高度精确地拟合其变化趋势；对于平均时延 $D$ 与公平性 $J_T$ 等更具复杂性的性能指标，模型同样保持了较低的 MSE 与 MAE，并取得了较高的 $R^2$，其中时延预测精度较高，而公平性指标由于各个样本之间的数值都相差不大，导致拟合优度略低。
...
\begin{figure}[t]
    ...
    \caption{均匀流量、归一化总到达率$\lambda=0.3$ 场景下的模型预测数值与仿真结果对比。节点数量为 $n=100$。时隙总长度为 $t=10^6$。}
...
本小节以均匀流量场景，即归一化总到达率 $\lambda=0.3$为例，对不同协议参数 $q$ 下的系统性能进行可视化对比分析。如图~\ref{fig:ai_vs_sim_uniform_003} 所示，实线表示多任务预测模型输出结果，离散点表示对应的仿真结果。可以观察到，在整个参数区间内，两者在吞吐量 $\lambda_{\mathrm{out}}$、时延 $\ln(1+D)$ 与信令开销 $S$ 三类指标上的变化趋势高度一致，且关键特征点，如饱和值与拐点位置，也能够良好对齐，表明所提出的多任务性能预测模型能够准确刻画协议参数与系统性能之间的非线性映射关系，具备较强的泛化能力与预测精度。
