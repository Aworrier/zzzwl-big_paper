...
不同随机接入协议在吞吐性能、时延特性、信令开销方面表现各异。表~\ref{tab:protocol_comparison} 对 SA、SAST、CBA三种基于时隙 Aloha 的协议进行定性对比，可以看出，协议机制的增强虽可显著提升系统吞吐，但同时也可能带来更高的信令开销，表明难以依靠单一协议适应所有场景。
...
在给定 $\mathbf{X}_{\mathrm{in}}$ 的前提下，本文通过统一仿真器或基于监督学习构建的性能预测模型来评估当前配置下的系统性能，两者分别记为映射 $S(\cdot)$或者映射 $f_{\Phi}(\cdot)$，其输出结果为多维性能指标向量 $\mathbf{y}$。为便于在不同场景与协议之间进行性能比对，引入归一化算子 $\mathrm{norm}(\cdot)$，得到归一化指标向量 $\tilde{\mathbf{y}}=\mathrm{norm}(\mathbf{y})$。在此基础上，进一步通过综合效用函数 $U(\cdot)$ 将 $\tilde{\mathbf{y}}$ 映射为单一的系统效用得分。
