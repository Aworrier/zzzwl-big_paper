...
\begin{figure}[htbp]
	...
	\subfloat[吞吐量 $\lambda_{\mathrm{out}}$]{
		\includegraphics[width=0.45\textwidth]{figure/chap5/throughput_vs_q_xprefix__lambda__0.003_traffic_type__uniform.png}
		\label{fig:ai_vs_sim_uniform_003_thr}
	}
	% (b) 平均时延
	\subfloat[平均时延 $D$]{
		\includegraphics[width=0.45\textwidth]{figure/chap5/delay_vs_q_xprefix__lambda__0.003_traffic_type__uniform.png}
		\label{fig:ai_vs_sim_uniform_003_delay}
	}\\
	% (c) 公平性
	\subfloat[公平性 $J_T$]{
		\includegraphics[width=0.45\textwidth]{figure/chap5/fairness_vs_q_xprefix__lambda__0.003_traffic_type__uniform.png}
		\label{fig:ai_vs_sim_uniform_003_fairness}
	}
	% (d) 信令开销
	\subfloat[信令开销 $S$]{
		\includegraphics[width=0.45\textwidth]{figure/chap5/signaling_overhead_vs_q_xprefix__lambda__0.003_traffic_type__uniform.png}
		\label{fig:ai_vs_sim_uniform_003_signaling}
	}
	\caption{单节点到达率 $\lambda=0.003$，流量类型为 uniform 场景下，AI代理模型预测曲线（实线）与真实仿真结果（离散点）的性能对比。\\
	(a) 吞吐量 $\lambda_{\mathrm{out}}$：展示了不同协议参数下系统的吞吐能力，AI模型预测与仿真结果高度吻合，说明模型能有效捕捉系统负载变化趋势。\\
	(b) 平均时延 $D$：反映了数据包在系统中的平均传输时延，AI预测曲线与仿真点基本重合，验证了模型对时延性能的准确刻画。\\
	(c) 公平性 $J_T$：衡量各节点资源分配的均衡性，结果显示AI模型能够较好地反映协议参数变化对公平性的影响。\\
	(d) 信令开销 $S$：描述了系统在不同参数下的信令消耗，AI预测与仿真结果趋势一致，表明模型在信令开销估计方面具有较高可靠性。}
